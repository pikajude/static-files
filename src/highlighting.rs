...
pub fn markdown(text: String) -> String {
    ...
    let prs = Parser::new_ext(text.as_str(), opts).filter_map(|event| match event {
        ...
        Event::End(Tag::CodeBlock(lang)) => {
            ...
            let mys = ss.find_syntax_by_extension(l_str.as_str())
            ...
            for line in code_buffer.as_str().lines() {
                html_str.push_str(tokens_to_classed_html(line,
                                                         ps.parse_line(line).as_slice(),
                                                         ClassStyle::Spaced)
                    .as_str());
                html_str.push('\n');
